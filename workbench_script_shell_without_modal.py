...
from models.wb_ansys_shell import WBShellModel
...
geometry_script_path_vertical = os.path.join(dir_path, 'scripts', 'geometry_creation_shell_vertical.py')
geometry_script_path_horizontal = os.path.join(dir_path, 'scripts', 'geometry_creation_shell_horizontal.py')
...
def update_project():
    ...
    update_mechanical_component(
        r'C:\Ansys projects\Lopast_helicopter\AnsysOptimization\scripts\geometry_creation_mass_and_mass_center_properties_shell.py',
    ...
    update_mechanical_component(
        r'C:\Ansys projects\Lopast_helicopter\AnsysOptimization\mechanikal_script_vertical_flight_shell_total.py',
...
for el in a.select_by_series('need_calculate'):
    current_object_list.append(WBShellModel(el))
...
for i, el in enumerate(current_object_list):
    ...
    number_of_layers = str(int(len(el.shell_integer_code) / 2))
